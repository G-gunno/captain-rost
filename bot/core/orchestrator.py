...
async def startup_reconciliation():
    ...
    for sym, pos in list(paper.positions.items()):
        ...
        if fixed:
            ...
            actions.append(f"{pair_html(sym[:-4], sector, kind_tag)} · {' · '.join(fixed)}")
    paper.save()
    ...
    for order in list(paper.orders):
        ...
        if not t:
            ...
            actions.append(f"{pair_html(sym[:-4], sector, kind_tag)} · ордер снят (нет данных)")
            continue
        candles = await market_data.get_kline(sym, "15", 120)
        if len(candles) < 60:
            continue
        ...
        if score < thr - 1:
            paper.cancel_order(order["id"])
            actions.append(f"{pair_html(sym[:-4], sector, kind_tag)} · ордер снят · ⭐ {score:.1f} ниже {thr:g}")
        elif a > 0:
            ...
            order["requotes"] = 0
            actions.append(f"{pair_html(sym[:-4], sector, kind_tag)} · ордер перевыставлен · ⭐ {score:.1f}")
    paper.save()
...
async def run_cycle():
    ...
    if drop_1h <= -3 or btc_t.get("change_pct", 0) <= -6:
        if paper.positions or paper.orders:
            for ex in paper.sell_all(tickers):
                await notify(
                    f"🚨 <b>Экстренный выход</b> · <b>{ex['symbol'][:-4]}</b> · "
    ...
    for sym, pos in list(paper.positions.items()):
        ...
        if neg > 0 and neg > pos_news:
            if pnl_pct >= MIN_EARLY_EXIT_PCT:
                ...
                await notify(
                    f"💸 <b>Продажа</b> · <b>{sym[:-4]}</b> · <i>{ex.get('sector', 'Other')}</i> · новостной выход ⚠️ {neg}\n"
                ...
                await notify(
                    f"💸 <b>Продажа</b> · <b>{sym[:-4]}</b> · <i>{ex.get('sector', 'Other')}</i> · новостная резка 🛑 {neg}\n"
                    f"{pnl_emoji(ex['pnl_pct'])} {fmt_pct(ex['pnl_pct'])} · {usd(ex['pnl'])} · 📊 {fmt_price(ex['price'])}"
                )
                continue
        ...
        if not pos.get("tp1_done") and last >= pos["tp"]:
            ...
            paper.save()
            funding_txt = f" · 🏦 +{usd(ex['transferred'])}" if ex["transferred"] > 0 else ""
            await notify(
                f"🎯 <b>TP1</b> · <b>{sym[:-4]}</b> · 50% · 🔥 {fmt_pct(ex['pnl_pct'])}\n"
                f"💵 {usd(ex['pnl'])}{funding_txt} · остаток бежит\n"
        ...
        if trend_broken or score_pos <= thr - 2:
            if pnl_pct >= MIN_EARLY_EXIT_PCT:
                ...
                await notify(
                    f"💸 <b>Продажа</b> · <b>{sym[:-4]}</b> · <i>{ex.get('sector', 'Other')}</i> · сигнал ослаб 📉\n"
                    f"{pnl_emoji(ex['pnl_pct'])} {fmt_pct(ex['pnl_pct'])} · {usd(ex['pnl'])} · 📊 {fmt_price(ex['price'])}"
                )
                continue
            if trend_broken and score_pos <= thr - 1 and pnl_pct <= -0.5:
                ...
                await notify(
                    f"💸 <b>Продажа</b> · <b>{sym[:-4]}</b> · <i>{ex.get('sector', 'Other')}</i> · резка убытка 🛑\n"
                    f"{pnl_emoji(ex['pnl_pct'])} {fmt_pct(ex['pnl_pct'])} · {usd(ex['pnl'])} · 📊 {fmt_price(ex['price'])}"
                )
                continue
        ...
        if pos.get("tp1_done") and last > e21 > e50 and score_pos >= thr and last >= pos["tp"] - 0.3 * a:
            ...
            if new_tp > pos["tp"]:
                ...
                await notify(
                    f"🎯 <b>TP поднят</b> (раннер) · <b>{sym[:-4]}</b>\n"
                    f"🎯 {fmt_price(pos['tp'])} · 🛡 {fmt_price(pos['sl'])}"
                )
    ...
    # 5. Выходы остатка по TP/SL: 💸 Продажа с ценой
    for ex in paper.check_exits(tickers):
        ...
        funding_txt = f"\n🏦 +{usd(ex['transferred'])} в накопления" if ex.get("transferred", 0) > 0 else ""
        await notify(
            f"💸 <b>Продажа</b> · <b>{ex['symbol'][:-4]}</b> · <i>{ex.get('sector', 'Other')}</i> · {ex['reason']}\n"
            f"{ind} {fmt_pct(ex['pnl_pct'])} · {usd(ex['pnl'])} · 📊 {fmt_price(ex['price'])}{runner_txt}{funding_txt}"
        )
    ...
    for order in list(paper.orders):
        ...
        if neg > 0 and neg > pos_news:
            paper.cancel_order(order["id"])
            await notify(f"⚠️ <b>Ордер снят</b> · <b>{base}</b> · негатив {neg}")
            continue
        ...
        if order.get("requotes", 0) >= 2:
            paper.cancel_order(order["id"])
            await notify(f"❌ <b>Ордер снят</b> · <b>{base}</b> · 3 попытки без исполнения")
            continue
        ...
        paper.save()
        await notify(
            f"🔁 <b>Ордер перевыставлен</b> · <b>{base}</b>\n"
